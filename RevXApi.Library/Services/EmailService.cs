...
namespace RevXApi.Library.Services
{
	public class EmailService : IEmailService
	{
		...
		public async Task SendEmail()
		{
			...
			sessions.Add(new() { Client = "First Last", Date = DateTime.Now, StartTime = new TimeSpan(), EndTime = new TimeSpan(), Notes = "Hello In the email." });
			sessions.Add(new() { Client = "First Last", Date = DateTime.Now.AddDays(-2), StartTime = new TimeSpan(), EndTime = new TimeSpan(), Notes = "second one." });
			sessions.Add(new() { Client = "First Last", Date = DateTime.Now, StartTime = new TimeSpan(), EndTime = new TimeSpan(), Notes = "Hello In the email." });
			sessions.Add(new() { Client = "First Last", Date = DateTime.Now, StartTime = new TimeSpan(), EndTime = new TimeSpan(), Notes = "Hello In the email." });
		...
		public async Task<SendResponse> SendInvoiceEmail(InvoiceEmailModel emailModel)
		{
			SendResponse email;
			if (emailModel.Signature != null && emailModel.Signature != String.Empty)
			{
				...
				email = await _fluentEmail
					.SetFrom(emailModel.FromAddress, emailModel.FullName)
					.To(emailModel.ToAddress)
					.CC(emailModel.CC)
