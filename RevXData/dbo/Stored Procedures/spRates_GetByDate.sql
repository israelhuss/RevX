...
BEGIN
	...
	INSERT INTO #RatesFromSp EXEC spRates_getAll @UserId = @UserId, @ProviderId = @ProviderId

	IF (@ProviderId is null) OR NOT EXISTS (SELECT * FROM #RatesFromSp
		WHERE StartDate <= @Date AND (EndDate is null or EndDate >= @Date) AND UserId = @UserId AND ProviderId = @ProviderId)
